...
import { renderMarkdown, addCodeBlockCopyButtons, formatMessage } from './markdown.js';
import { readFileAsBase64, getSelectedFiles, clearSelectedFiles } from './fileHandler.js';
import { callOpenAIAPI } from './api.js';
import { saveConversations } from './storage.js';

// ユーザーメッセージを追加する関数
export function addUserMessage(message, chatMessages) {
...
// ボットメッセージを追加する関数
export function addBotMessage(message, chatMessages) {
    const messageDiv = document.createElement('div');
...
// 会話を表示
export function displayConversation(conversation, chatMessages, modelSelect) {
    ...
    conversation.messages.forEach(message => {
        ...
        if (message.role === 'user') {
            addUserMessage(message.content, chatMessages);
        } else if (message.role === 'assistant') {
            addBotMessage(message.content, chatMessages);
        }
    });
...
// メッセージを送信する関数
export async function sendMessage(userInput, chatMessages, currentConversation, apiSettings, systemPrompt) {
    ...
    if (message || selectedFiles.length > 0) {
        ...
        // ユーザーメッセージを表示
        addUserMessage(messageContent, chatMessages);
        userInput.value = '';
        ...
        try {
            ...
            // API呼び出し
            const botResponse = await callOpenAIAPI(
                messagesWithSystem, 
...
// 会話履歴を表示する
export function renderChatHistory(conversations, currentConversationId, chatHistory, onSwitchConversation, onShowRenameModal, onDeleteConversation) {
...
// アクティブなチャットを更新
export function updateActiveChatInHistory(currentConversationId) {
